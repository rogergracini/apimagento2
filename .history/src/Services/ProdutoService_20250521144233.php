    ...
    public function importarProduto(array $produto): array
{
    ...
    // Verifica se o produto já existe no Magento
    $produtoExistente = $this->buscarProdutoMagento($sku);
    if ($produtoExistente && isset($produtoExist->id)) {
        return ['status' => 'ignorado', 'mensagem' => 'Produto já existe no Magento'];
    }
    ...
    $payload = [
        ...
        // Adiciona os preços por grupo ao payload
        'tier_prices' => $calculo['tier_prices']
    ];

    // Envia o payload para o Magento
